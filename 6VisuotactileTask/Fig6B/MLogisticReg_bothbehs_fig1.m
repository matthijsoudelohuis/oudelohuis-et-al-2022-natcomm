%% Multinomial Logistic Regression - Two regressors - tMulti-allbeh-fig1
...
%% Pre-processing
%Cut trials <200 trials
trialData.RT = nan(length(trialData.stimStart),1);
for k =1:length(trialData.RT)
    LTIW = trialData.lickTime{k}(trialData.lickTime{k}>=trialData.respwinStart(k));
    if ~isempty(LTIW)
...
for k=1:length(tfields) %clean trials that didnt happen
        if (isnumeric( trialData.(tfields{k}) ) || iscell( trialData.(tfields{k}) ) || islogical(trialData.(tfields{k}) ))
            trialData.(tfields{k})(trialData.RT<=0.2) = [];
        end
end
...
if strcmp(SelPred, 'Visual')
    tfields = fieldnames(trialData);
    filt = (trialData.trialType=='V' | trialData.trialType=='C');
    for k=1:length(tfields) 
        if (isnumeric( trialData.(tfields{k}) ) || iscell( trialData.(tfields{k}) ) || islogical(trialData.(tfields{k}) ))
            trialData.(tfields{k}) = trialData.(tfields{k})(filt);
        end
    end
    %Remove contrasts >26 for consistency
%     filt2 = (abs(trialData.stimContrast)<=0.26);
%     for k=1:length(tfields) 
%         if (isnumeric( trialData.(tfields{k}) ) || iscell( trialData.(tfields{k}) ) || islogical(trialData.(tfields{k}) ))
%             trialData.(tfields{k}) = trialData.(tfields{k})(filt2);
%         end
%     end
    predictor = 100*trialData.stimContrast;
elseif strcmp(SelPred, 'Tactile')
    tfields = fieldnames(trialData);
    filt = (trialData.trialType=='T' | trialData.trialType=='C');
    for k=1:length(tfields) %clean trials that didnt happen
        if (isnumeric( trialData.(tfields{k}) ) || iscell( trialData.(tfields{k}) ) || islogical(trialData.(tfields{k}) ))
            trialData.(tfields{k}) = trialData.(tfields{k})(filt);
        end
...
%save(strcat('MLogisticReg_',SelPred,'_nomaxcut_',sessionData.Mouse(1:4)),'n','stimCombs','Y','B','trialData','sessionData','MergedStr','xR','xL','pR','pL');