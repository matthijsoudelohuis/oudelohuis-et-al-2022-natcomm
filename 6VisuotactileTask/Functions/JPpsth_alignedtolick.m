function  [frpertrial_all, tempsms, FRz_all, RT_all, findem, rawspikes] = JPpsth_alignedtolick(files, SelArea, SelConds, SelTrialOutcome, SelLick, varargin)
%Same as JPpsth_fig6c but aligns to lick instead.
%SelLick = [1 0] selects only licks to the left
%SelLick = [0 1] selects only licks to the right
%SelLick = [1 1] selects both
...
for iSess=1:length(files)
    clearvars -except iSess files kk win FRz_all sortmi* RT* frpertrial_all SelArea SelConds SelTrialOutcome SelMouseChoice findem Removepre240 rawspikes SelLick
